...
if(empty($_POST)) {
	...
	if (!isset($_POST['id'])  || !is_numeric($_POST['id']) ){
		...
		if ($stmt) {
			...
			//結果変数のバインド
			$stmt->bind_result($id,$name,$gender,$date,$pref);
...
<html>
	...
	<body>
		...
		<h2>性別</h2>
			<select name="gender" value="<?=htmlspecialchars($gender, ENT_QUOTES, 'UTF-8')?>" required>
				<option></option>
		...
		<h2>地域</h2>
			<select name="pref" value="<?=htmlspecialchars($pref, ENT_QUOTES, 'UTF-8')?>" required>
				<option value="">-</option>
